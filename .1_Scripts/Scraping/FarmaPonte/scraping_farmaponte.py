...
def descobrir_total_paginas():
    print("🔭 FASE 0: Mapeando o tamanho do catálogo da FarmaPonte...")
    url_base = "https://www.farmaponte.com.br/saude/medicamentos/?p=1"
    ...
    try:
        ...
        if texto_paginacao:
            ...
            if match:
                ...
                print(f"✅ Catálogo mapeado com sucesso: {total} páginas de medicamentos encontradas.")
                return total
...
def extrair_links_da_pagina(pagina):
    base_url = "https://www.farmaponte.com.br/saude/medicamentos/"
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
    ...
    try:
        ...
        for prod in produtos:
            ...
            if tag_a and 'href' in tag_a.attrs:
                link_completo = "https://www.farmaponte.com.br" + tag_a['href']
...
# MOTOR PRINCIPAL
# ==========================================
def executar_spider_farmaponte():
    nome_arquivo = 'Scraping_FarmaPonte.csv'
    ...
    # ----------------------------------------------------
    # NOVA CHAMADA: O script descobre o total de páginas sozinho
    # ----------------------------------------------------
    ...
    print(f"Alvo: {len(links_para_raspar)} produtos. Salvando em '/home/ubuntu/.2_Dados/FarmaPonte/{nome_arquivo}'.")
    print("-" * 60)
    
    # Salvando na raiz do diretório onde o script está rodando
    with open(f"/home/ubuntu/.2_Dados/FarmaPonte/{nome_arquivo}", mode='w', newline='', encoding='utf-8-sig') as arquivo_csv:
    ...
    print(f"\n✅ Scraping Arquitetura Spider finalizado! Base autônoma da FarmaPonte gerada.")

if __name__ == "__main__":
    executar_spider_farmaponte()